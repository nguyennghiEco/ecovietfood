...
<div class="page-wrapper">
  ...
  <div>
    ...
    <div class="form-group col-12 text-center btm-btn">
        ...
        <a href="javascript:history.back()" class="btn btn-default"><i class="fa fa-undo"></i>{{ trans('lang.cancel')}}</a>
        </div>
</div>
@endsection
@section('scripts')
...
$(document).ready(function(){
  ...
  ref.get().then( async function(snapshots){
    ...
    try{
      ...
      if(book.guestEmail !="" && book.guestEmail!=null){
        $(".guest_email").val(shortEmail(book.guestEmail));
      }
      else
      {
        $(".guest_email").val("");
      }
      if(book.guestPhone !="" && book.guestPhone!=null){
        $(".guest_phone").val(shortEditNumber(book.guestPhone));
      }
      else
      {
        $(".guest_phone").val("");
