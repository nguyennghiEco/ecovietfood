...
<script type="text/javascript">
    ...
    var month = (curr_date.getMonth() + 1).toString().padStart(2, '0');  // Month is 0-based, so add 1
    var day = curr_date.getDate().toString().padStart(2, '0');           // Pad with leading zero if necessary
    var hours = curr_date.getHours().toString().padStart(2, '0');         // Pad with leading zero if necessary
    var minutes = curr_date.getMinutes().toString().padStart(2, '0');     // Pad with leading zero if necessary

    // Construct the formatted string: "YYYY-MM-DD HH:mm"
    var curr_date_utc = year + '-' + month + '-' + day + ' ' + hours + ':' + minutes;
...
var history_request= database.collection('booked_table').where("author.id","==",user_uuid).where("date","<",createdAtman).orderBy('date', 'desc');
    ...
    function buildHTMLCompletedOrders(completedorderSnapshots) {
        ...
        if (alldata.length === 0) {
            html = '<div class="no-records">{{ trans('lang.no_results') }}</div>';
        }
        alldata.forEach((listval) => {
            ...
            html = html +
                ...
                placeholderImage + '\'" alt="#" src="' + orderRestaurantImage +
        ...
        alldata.forEach((listval) => {
            ...
            if (val.status == "Order Rejected") {
                ...
                html = html +
                    ...
                    '</a></p><p class="mb-0"><span class="fa fa-map-marker"></span> ' + val.vendor.location +
                    '</p><p>ORDER ' + val.id +
                    '</p></div><div class="ml-auto ord-com-btn"><p class="bg-rejected text-white py-1 px-2 rounded small mb-1">Rejected</p><p class="small font-weight-bold text-center"><i class="feather-clock"></i> ' +
