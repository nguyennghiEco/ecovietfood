                    ...
                    <?php if ($id == '') { ?>
                        <div class="form-group row width-100">
                            <label class="col-3 control-label">{{
                                trans('lang.restaurants_payout_restaurant')}}</label>
                            <div class="col-7">
...
<script>
        ...
        database.collection('vendors').orderBy('title', 'asc').get().then(async function (snapshots) {
            snapshots.docs.forEach((listval) => {
                ...
                $('#select_restaurant').append($("<option></option>")
                    .attr("value", data.id)
                    .text(data.title ? data.title : data.authorName));
            })
        });
    ...
    $(".save-form-btn").click(async function () {
            ...
            if(vendorID == ''){
                ...
                $(window).scrollTop(0);
                $(".error_top").html("");
                $(".error_top").append("<p>{{trans('lang.restaurant_vendor')}}</p>");
            } else if(isNaN(amount)  || amount == "" || amount == null){
                $(".error_top").show();
                $(window).scrollTop(0);
                $(".error_top").html("");
                $(".error_top").append("<p>{{trans('lang.restaurant_valid_payment')}}</p>");
            } else if(note == ''){
                $(".error_top").show();
                $(window).scrollTop(0);
                $(".error_top").html("");
                $(".error_top").append("<p>{{trans('lang.restaurant_note_error')}}</p>");
                 ...
                 var remaining = await remainingPrice(vendorID);
                    if(remaining < amount){
                        $(".error_top").show().html("<p>{{trans('lang.restaurant_insufficient_payment_error')}}</p>");
                        window.scrollTo(0, 0);
                        return;
                    }else {
